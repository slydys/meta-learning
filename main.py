...
def train_model(model, train_loader, criterion, optimizer, num_epochs=80, save_path='inception_v3_model.pth'):
    model.train()
...
def load_model(model, optimizer, load_path='inception_v3_model.pth'):
    checkpoint = torch.load(load_path)
    model.load_state_dict(checkpoint['model_state_dict'])
    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    epoch = checkpoint['epoch']
    loss = checkpoint['loss']
    print(f"Model loaded from {load_path}, trained for {epoch} epochs, loss: {loss}")
...
def main(dataset_folder):
    ...
    train_model(model, train_loader, criterion, optimizer, num_epochs=80, save_path='inception_v3_model.pth')
